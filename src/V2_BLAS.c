...
void kNN(double * X, double * Y, int n, int m, int d, int k, knnresult* result, int step, VPT *T)
{

	int rank = 0;
	
	// For every point in query set Y
	// find k nearest points from corpus set X using VPT
	for(int i = 0; i < m; ++i) {

		searchVPT(T, Y+i*d, d, result->ndist, result->nidx);
...
knnresult distrAllkNN(double* X, int n, int d, int k)
{
  ...
  // get number of
	...
	if(rank == 0) {
		...
		VPT* tree = (VPT*)malloc(1*sizeof(VPT));
		makeVPT(tree, Xlocal, n, d);
	...
	for(int i = 0; i < numtasks; ++i) {
		...
		// Calculate
		kNN(Y, Xlocal, chunk_size[rank], chunk_size[rank], d, k, result, i, tree);
